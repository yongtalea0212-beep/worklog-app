...
export default function LoginPage() {
  ...
  async function handleEmail(e) {
    ...
    try {
      if (mode === 'login') {
        ...
        // Full reload so WorklogApp gets fresh session
        window.location.href = '/'
      } else if (mode === 'signup') {
  ...
  if (checking) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(160deg,#F0ECFF,#E8F4FF,#F0FFF8)',fontFamily:'Inter,sans-serif',fontSize:14,color:'#6C63FF'}}>
      ✨ กำลังตรวจสอบ...
    </div>
  )
  ...
  const CSS = `
    ...
    * { box-sizing: border-box; }
    ...
    .login-brand-sub { font-size:11px; color:#9ca3af; }
    ...
    .login-btn {
      ...
      box-shadow:0 4px 16px rgba(108,99,255,0.35); margin-top:4px;
    }
    ...
    .login-divider::before,.login-divider::after {
      content:''; flex:1; height:1px; background:rgba(200,210,240,0.5);
    }
    .oauth-btn {
      width:100%; padding:11px 14px;
      background:rgba(255,255,255,0.7);
      ...
      <div className="login-root">
        <div className="login-card">
          <div className="login-logo">
            <div className="login-logo-mark"><span>SS</span></div>
            <div>
          ...
          <div className="login-sub">
            {mode==='login'?'เข้าสู่ระบบเพื่อเริ่มบันทึกงาน':mode==='signup'?'เริ่มต้นใช้งาน StayScape ฟรี':'ใส่อีเมลเพื่อรับ link reset password'}
          </div>
          ...
          <form onSubmit={handleEmail}>
            {mode==='signup' && (
              <div className="login-input-wrap">
                ...
                <input className="login-input" type="text" placeholder="ชื่อของคุณ" value={name} onChange={e=>setName(e.target.value)} required/>
              </div>
            )}
            <div className="login-input-wrap">
              ...
              <>
                ...
                {mode==='login' && (
                  <div className="login-forgot">
                    <button type="button" onClick={()=>{setMode('forgot');setError('');setSuccess('')}}>ลืมรหัสผ่าน?</button>
                  </div>
                )}
              </>
            )}
          ...
          {mode!=='forgot' && (
            <>
              ...
              <button className="oauth-btn" onClick={handleLine} disabled={loading} style={{borderColor:'rgba(6,199,85,0.3)',color:'#059669'}}>
